...
def light_up():
    subprocess.call(["challenge3Monitor"])

msg = ('Remember to save your JSON file after changes.'
        'Good Luck !!!'
)

# If we want to test the policy, ie, publish to only allowed client or topic, can use below:
...
def client_pub():
    ...
    #Publish topic
    #myAWSIoTMQTTClient.publish(topic, json.dumps({"challenge1": "on"}), 1)
    ...
    #   ------   TODO    ----------
    #  update the topic below
   # message should be 
   #  lightOnMessage  is   "<group-number>":{"challenge3":"on"} for light on 
   #  lightOffMessage is  "<group-number>":{"challenge3":"off"} for light off
   #  Generate reported statement for shadow. 

    topic = ""
    lightOnMessage = ""
    lightOffMessage = ""
     
    print('------Please check your group challenge 3 light status-----')	
    myAWSIoTMQTTClient.publish(topic, lightOnMessage, 1)
    time.sleep(5)
    myAWSIoTMQTTClient.publish(topic, lightOffMessage, 1)
    time.sleep(5)
    myAWSIoTMQTTClient.publish(topic, lightOnMessage, 1)
    time.sleep(5)
    myAWSIoTMQTTClient.publish(topic, lightOffMessage, 1)
