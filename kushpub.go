...
// MaterPub: func to be exported, and used for main sub func
func MaterPub() {
	...
	opts.AddBroker("ssl://XXXXXXXXXXX-ats.iot.ap-southeast-2.amazonaws.com:8883")
	...
	if token := c.Connect(); token.Wait() && token.Error() != nil {
		...
		if groupConfig == "group1" {
			shadowOn := `{"state":{"reported":{"group1":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group2" {
			shadowOn := `{"state":{"reported":{"group2":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group3" {
			shadowOn := `{"state":{"reported":{"group3":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group4" {
			shadowOn := `{"state":{"reported":{"group4":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group5" {
			shadowOn := `{"state":{"reported":{"group5":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group6" {
			shadowOn := `{"state":{"reported":{"group6":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group7" {
			shadowOn := `{"state":{"reported":{"group7":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group8" {
			shadowOn := `{"state":{"reported":{"group8":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group9" {
			shadowOn := `{"state":{"reported":{"group9":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else if groupConfig == "group10" {
			shadowOn := `{"state":{"reported":{"group10":{"challenge1":"on"}}}}`
			c.Publish("$aws/things/demo/shadow/update", 1, false, shadowOn)
			c.Disconnect(250)
			fmt.Println("\nMeesage has been published")
		} else {
